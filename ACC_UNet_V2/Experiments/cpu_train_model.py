...
from nets.ACC_UNet_W import ACC_UNet_W
from nets.ACC_UNet_Lite import ACC_UNet_Lite

from torch.utils.data import DataLoader
import logging
import json
from cpu_train_one_epoch import train_one_epoch
...
def main_loop(batch_size=config.batch_size, model_type='', tensorboard=True):
    ...
    if model_type == 'ACC_UNet':
        ...
        config_vit = config.get_CTranS_config()        
        model = ACC_UNet_Lite(n_channels=config.n_channels,n_classes=config.n_labels)
        lr = 5e-4

    elif model_type == 'ACC_UNet_W':
        config_vit = config.get_CTranS_config()        
        model = ACC_UNet_W(n_channels=config.n_channels,n_classes=config.n_labels)
        lr = 5e-4
    ...
    # logger.info('Training on ' +str(platform.node()))
